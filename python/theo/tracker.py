...
class ReID_Tracker():
    def __init__(self, path, metric, dist_thresh, ref_method, nb_ref, av_method, intra_dist, verbose):
        self.ReID_model=ResNet50(751)
    ...
    def embedding_comparator(self, detections):
        ...
        if self.ref_emb.nelement() == 0:
            ...
            if self.verbose is True: print("list of distances to ref:", dist_list)
            if metric=='L2':
    ...
    def average_ref(self):
        ...
        if(method=='standard'):
            ...
            print("ERROR !! method of averaging not correct")
            sys.exit()
...
#from ..src.dlav22.deep_sort.deep.reid.torchreid.models import resnet50_ibn_a

# class new_ResNet101(ReID_Tracker):
#     def __init__(self, path, metric, dist_thresh, ref_method, nb_ref, av_method, intra_dist, verbose):
#         #self.ReID_model=ResNet50(751)
#         self.ReID_model = resnet50_ibn_a(1000)#torch.hub.load('XingangPan/IBN-Net', 'resnet101_ibn_a', pretrained=False)
#         #model.eval()
#         self.transformation=transforms.Compose([
#             transforms.Resize((256,128)),
#             transforms.ToTensor(),
